...
public class GameManager : MonoBehaviour
{
    ...
    public IEnumerator SaveData(PlayerData data)
    {
        using (StreamWriter stream = new StreamWriter(
            Path.Combine(Application.persistentDataPath, "savedata_encrypt_.json"),
            false, System.Text.Encoding.UTF8))
        {
            //DataManipulator manipulator = new DataManipulator();
            stream.Write(/*manipulator.Encrypt(*/PlayerDataJson.WriteJson(data)/*)*/);
            Debug.Log(Path.Combine(Application.persistentDataPath, "savedata_encrypt_.json"));
        }
    ...
    {
        string path = Path.Combine(Application.persistentDataPath, "savedata_encrypt_.json");
        if (File.Exists(path))
        {
